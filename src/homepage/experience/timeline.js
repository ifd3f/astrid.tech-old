...
function buildGantt(intervals) {
  ...
  for (var i = 1; i < intervals.length; i++) {
    ...
    if (!createdNewLayer) {
      // If we haven't created a new layer
    ...
    for (const j in supersetLayers) {
      // Add in the layers we sifted through
...
export function TimelineLayer({ children, width = 30 }) {
  ...
  const layers = gantt.map((layer, i) => (
    <Col key={i}>{layer.map((x) => x.value)}</Col>
  ));

  return <Row>{layers}</Row>;
}
